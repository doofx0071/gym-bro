...
export default function WorkoutPlansPage() {
  ...
  const handleRegenerate = async (planId: string, e: React.MouseEvent) => {
    e.stopPropagation()
    try {
      const response = await fetch(`/api/workout-plans/${planId}/regenerate`, {
        method: 'POST',
      })
      ...
      toast.success('Regenerating workout plan...')
      router.push(`/workout-plans/${planId}?generating=true`)
    } catch (err) {
  ...
  const handleDelete = async (planId: string) => {
    ...
    try {
      const response = await fetch(`/api/workout-plans/${planId}/delete`, {
        method: 'DELETE',
      })
